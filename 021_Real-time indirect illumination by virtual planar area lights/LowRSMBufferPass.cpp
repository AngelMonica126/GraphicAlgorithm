...
CLowRSMBufferPass::CLowRSMBufferPass(const std::string& vPassName, int vExcutionOrder) : IRenderPass(vPassName, vExcutionOrder)
{
}
...
{
	m_pShader = std::make_shared<CShader>("LowRSMBuffer_VS.glsl", "LowRSMBuffer_FS.glsl");
	m_pSponza = std::dynamic_pointer_cast<CSponza>(ElayGraphics::ResourceManager::getGameObjectByName("Sponza"));

	m_pDynamicObjectShader = std::make_shared<CShader>("LowRSMBuffer_VS.glsl", "LowRSMBuffer_FS.glsl");
	...
	TextureConfig4Position->InternalFormat = GL_RGBA32F;
	TextureConfig4Position->ExternalFormat = GL_RGBA;
	TextureConfig4Position->DataType = GL_FLOAT;
	TextureConfig4Position->Width = m_Block;
	TextureConfig4Position->Height = m_Block;
	TextureConfig4Position->Type4WrapT = GL_CLAMP_TO_EDGE;
	TextureConfig4Position->Type4WrapS = GL_CLAMP_TO_EDGE;
	TextureConfig4Position->Type4MinFilter = GL_NEAREST_MIPMAP_NEAREST;
	TextureConfig4Position->Type4MagFilter = GL_NEAREST;
	TextureConfig4Position->isMipmap = true;
	genTexture(TextureConfig4Position);

	m_FBO = genFBO({TextureConfig4Position});

	ElayGraphics::ResourceManager::registerSharedData("PlaneCentreTexture", TextureConfig4Position);
	...
	ElayGraphics::ResourceManager::registerSharedData("Block", m_Block);
}
