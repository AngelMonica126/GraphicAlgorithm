...
//	AlbedoAndMetallic_ = texture(u_DiffuseTexture, v2f_TexCoords);
	...
	if(alpha != 1.0f)
		discard;
//	vec3 N = normalize(mat3(u_TransposeInverseViewModelMatrix) * v2f_Normal);
//	vec3 T = normalize(mat3(u_TransposeInverseViewModelMatrix) * v2f_Tangent);
//	vec3 Bitangent = normalize(cross(N, T));
//	mat3 TBNMatrix = mat3(T, Bitangent, N);
//	NormalAndDoubleRoughness_ = vec4(FetchNormal(v2f_TexCoords,TBNMatrix),0);
