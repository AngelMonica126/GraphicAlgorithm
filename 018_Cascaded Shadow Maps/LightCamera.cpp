...
void CLightCamera::setFrustum()
{
	...
	for (int i = 0; i < m_SplitNum; i++) {
		...
		float NearHeight = glm::tan(glm::radians(FOVHalf)) * t_frustum.Near;
		float NearWidth = NearHeight * Ratio;
		float FarHeight = glm::tan(glm::radians(FOVHalf)) * t_frustum.Far;
...
{
	initFrustum();
	ElayGraphics::Camera::setMainCameraFarPlane(150.0f);
	ElayGraphics::Camera::setMainCameraPos({ 0, 39.1055, 62.8284 });
	ElayGraphics::Camera::setMainCameraFront({ 0, -0, -1});
