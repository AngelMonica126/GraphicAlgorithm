...
void CScreenQuadPass::updateV()
{
	...
	if (ElayGraphics::InputManager::getKeyStatus(GLFW_KEY_L) == GLFW_PRESS && m_OldKeyMFFSR != GLFW_PRESS)
	{
		m_OldKeyMFFSR = GLFW_PRESS;
		m_pShader->setTextureUniformValue("u_InputTexture", ElayGraphics::ResourceManager::getSharedDataByName<std::shared_ptr<ElayGraphics::STexture>>("MFEASUTexture"));
		std::cout << "Mobile Friendly FSR " << std::endl;
	}
	else if (ElayGraphics::InputManager::getKeyStatus(GLFW_KEY_L) == GLFW_RELEASE)
		m_OldKeyMFFSR = GLFW_RELEASE;
