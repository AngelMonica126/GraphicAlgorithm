...
CESMBufferPass::CESMBufferPass(const std::string& vPassName, int vExcutionOrder) : IRenderPass(vPassName, vExcutionOrder)
{
}

CESMBufferPass::~CESMBufferPass()
{
}

void CESMBufferPass::initV()
{
	m_pShader = std::make_shared<CShader>("ESMBuffer_VS.glsl", "ESMBuffer_FS.glsl");
	m_pSponza = std::dynamic_pointer_cast<CSponza>(ElayGraphics::ResourceManager::getGameObjectByName("Sponza"));
	m_pDynamicObjectShader = std::make_shared<CShader>("ESMBuffer_VS.glsl", "ESMBuffer_FS.glsl");
