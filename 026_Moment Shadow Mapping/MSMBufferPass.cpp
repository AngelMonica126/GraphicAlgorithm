...
CMSMBufferPass::CMSMBufferPass(const std::string& vPassName, int vExcutionOrder) : IRenderPass(vPassName, vExcutionOrder)
{
}

CMSMBufferPass::~CMSMBufferPass()
{
}

void CMSMBufferPass::initV()
{
	m_pShader = std::make_shared<CShader>("MSMBuffer_VS.glsl", "MSMBuffer_FS.glsl");
	m_pSponza = std::dynamic_pointer_cast<CSponza>(ElayGraphics::ResourceManager::getGameObjectByName("Sponza"));
	m_pDynamicObjectShader = std::make_shared<CShader>("MSMBuffer_VS.glsl", "MSMBuffer_FS.glsl");
